...
**선행 지식**: [에이전트형 인공지능](/book/24-agents), [에이전트: 도구 사용과 실행 제어](/book/24-agents)
...
**안전장치는 처음부터.** 이 짧은 코드에도 세 겹이 있다: 최대 턴 수(폭주 방지), 경로 샌드박스(디렉토리 탈출 방지), `eval` 대신 AST 화이트리스트(코드 주입 방지). 쓰기·삭제·외부 API처럼 되돌리기 어려운 도구에는 사람 승인 단계를 넣는다. ([실행 제어](/book/24-agents))
