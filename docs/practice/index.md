...
이론을 아는 것과 코드를 짜는 것 사이의 간극을 메우는 트랙이다. 각 문서는 **처음부터 끝까지 실행 가능한 완전한 코드**를 중심으로, 밑바닥 구현(라이브러리 없이 원리 확인)과 실전 파이프라인(현업 도구 사용)을 모두 다룬다. [책](/book/)에서 원리를 이해한 뒤 여기서 손을 움직인다. 각 코드랩 머리의 "선행 지식"이 먼저 읽을 장을 가리킨다.
